" -*- vim -*-
" FILE: "/home/wlee/vim/vimfiles/plugin/DirDiff.vim" {{{
" LAST MODIFICATION: "Tue, 09 Oct 2001 12:26:39 -0700 (wlee)"
...
"   Salman Harlim, Yosuke Kimura, and others for their suggestions
"
" HISTORY:
...
"    let addarg = input("Additional diff args (current =". cmdarg. "): ")
    ...
    call append(2, "Usage: <Enter>/'o'=open,'s'=sync,'n'=next,'p'=prev,'u'=update,'x'=excludes,'i'=ignore" )
    call append(3, "Diff Args:" . cmdarg)
    call append(4, "")
...
function! <SID>DirDiffNext()
    ...
    " if the current line is <= 5, (within the header range), we go to the
...
function! <SID>ChangeExcludes()
    let g:DirDiffExcludes = input ("Exclude pattern (separate multiple patterns with ','): ")
    echo "\nPress update ('u') to refresh the diff."
endfunction

" Let you modify the Ignore patthern
function! <SID>ChangeIgnore()
    let g:DirDiffIgnore = input ("Ignore pattern (separate multiple patterns with ','): ")
    echo "\nPress update ('u') to refresh the diff."
endfunction
